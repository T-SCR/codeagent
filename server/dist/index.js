...
async function getFileContentSnippet(bucket, pathStr, type) {
    try {
...
function getLocalFilesList() {
    ...
    return files.map((filename) => {
        ...
        return {
            ...
            is_default: false,
            uploaded_by: null,
            created_at: fs_1.default.statSync(path_1.default.join(LOCAL_FILES_DIR, filename)).ctime,
        };
    });
}
async function getLocalFileContentSnippet(filename, type) {
...
app.get('/api/local-files/download', (req, res) => {
    ...
    const filePath = path_1.default.join(LOCAL_FILES_DIR, String(name));
    if (!fs_1.default.existsSync(filePath))
...
app.post('/api/chat-history', (req, res) => {
    ...
    if (fs_1.default.existsSync(CHAT_HISTORY_FILE)) {
        history = JSON.parse(fs_1.default.readFileSync(CHAT_HISTORY_FILE, 'utf-8'));
    }
    history.push({ user, question, answer, timestamp: timestamp || new Date().toISOString() });
    fs_1.default.writeFileSync(CHAT_HISTORY_FILE, JSON.stringify(history, null, 2));
    res.json({ success: true });
});
app.get('/api/chat-history', (req, res) => {
    if (!fs_1.default.existsSync(CHAT_HISTORY_FILE))
        return res.json({ history: [] });
    const history = JSON.parse(fs_1.default.readFileSync(CHAT_HISTORY_FILE, 'utf-8'));
    res.json({ history });
});
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        ...
        const relevantFiles = allFiles.filter((f) => f.filename?.toLowerCase().includes(keyword) ||
        ...
        const fileInfos = await Promise.all(filesToProcess.map(async (f) => {
            let snippet = '';
            let downloadUrl = '';
            if (f.bucket === 'local') {
